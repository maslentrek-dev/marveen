...
function sendPendingRetryAlert(view: PendingRetryView, nowMs: number): void {
  ...
  const text = (mcpMissing
    ? [
        `[${BOT_NAME} scheduler]${repeat ? ' (ISMÉTELT riasztás)' : ''} A(z) "${view.taskName}" (${view.agentName}) feladat NEM tud lefutni: a szükséges MCP szerver(ek) nem futnak a cél-sessionben: ${mcpMissing}.`,
    ...
    : [
        `[${BOT_NAME} scheduler]${repeat ? ' (ISMÉTELT riasztás)' : ''} A(z) "${view.taskName}" (${view.agentName}) ütemezett feladat ${ageMinutes} perce várakozik (${view.attemptCount} próbálkozás).`,
